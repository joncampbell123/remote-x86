...
static struct rtl8139_rx_head	*rtl8139_rx=NULL;
static unsigned char		**rtl8139_rx_packets;
static struct rtl8139_tx_head	*rtl8139_tx=NULL;
static unsigned char		**rtl8139_tx_packets;
static unsigned int		rtl8139_ring_size;
static unsigned int		rtl8139_rx_offset;
static unsigned int		rtl8139_tx_offset;
...
static int rtl8139_init(struct pci_device *pdev) {
	...
	io_outl(rtl8139_iobase+0x44,0x000F);	/* accept bcast+mcast+physmatch+physaddr */
	...
	align_alloc(256);
	rtl8139_ring_size = 4;
	if (rtl8139_rx == NULL) rtl8139_rx = do_alloc(rtl8139_ring_size*16);
	if (rtl8139_tx == NULL) rtl8139_tx = do_alloc(rtl8139_ring_size*16);
	if (rtl8139_rx_packets == NULL) rtl8139_rx_packets = do_alloc(4*sizeof(unsigned char*));
	if (rtl8139_tx_packets == NULL) rtl8139_tx_packets = do_alloc(4*sizeof(unsigned char*));

	/* set up ring buffer */
	memset(rtl8139_rx,0,rtl8139_ring_size*16);
	memset(rtl8139_tx,0,rtl8139_ring_size*16);

	memset(rtl8139_rx_packets,0,rtl8139_ring_size*sizeof(unsigned char*));
	memset(rtl8139_tx_packets,0,rtl8139_ring_size*sizeof(unsigned char*));

	/* prepare rx for receiving data */
	align_alloc(32);
	for (i=0;i < rtl8139_ring_size;i++)
		rtl8139_rx_packets[i] = do_alloc(1600);
	for (i=0;i < rtl8139_ring_size;i++)
		rtl8139_tx_packets[i] = do_alloc(1600);

	for (i=0;i < rtl8139_ring_size;i++) {
		rtl8139_rx[i].addr = (uint64_t)((size_t)rtl8139_rx_packets[i]);
		rtl8139_rx[i].d2 = 0x00000000;
		rtl8139_rx[i].d1 = 0x80000000 | (i == (rtl8139_ring_size - 1) ? 0x40000000 : 0) | 1600;	/* set EOR end-of-ring for desc 3, and NIC owns it now */
	}

	for (i=0;i < rtl8139_ring_size;i++) {
		rtl8139_tx[i].addr = (uint64_t)((size_t)rtl8139_tx_packets[i]);
		rtl8139_tx[i].d2 = 0x00000000;
		rtl8139_tx[i].d1 = 0x00000000 | (i == (rtl8139_ring_size - 1) ? 0x40000000 : 0) | 1600;	/* set EOR end-of-ring for desc 3, and I own it */
	}
	...
	io_outl(rtl8139_iobase+0xE8,0);
	io_outl(rtl8139_iobase+0xE4,(uint32_t)rtl8139_rx);
	io_outl(rtl8139_iobase+0x20,(uint32_t)rtl8139_tx);
	io_outl(rtl8139_iobase+0x24,0);
	io_outl(rtl8139_iobase+0x28,0);
	io_outl(rtl8139_iobase+0x2C,0);
...
static void rtl8139_rx_reinit(struct rtl8139_rx_head *rx,int entry) {
	rx->d2 = 0x00000000;
	rx->d1 &= 0x3FFFC000;
	rx->addr = (uint64_t)((size_t)rtl8139_rx_packets[entry]);
	/* FIX: is the stupid NIC resetting the "last descriptor" bit?? */
	if (entry == (rtl8139_ring_size-1)) rx->d1 |= 0x40000000;
	rx->d1 |= 0x80000000 | 1600;	/* set EOR end-of-ring for desc 3, and NIC owns it now */
}

static void rtl8139_adv_packet() {
	if (++rtl8139_rx_offset >= rtl8139_ring_size)
		rtl8139_rx_offset = 0;
}
